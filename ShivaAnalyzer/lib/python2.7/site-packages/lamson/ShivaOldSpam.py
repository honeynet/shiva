...
def pushIntoDB(mailFields, key, msgMailRequest, exeSql):
  ...
  #logging.critical("checkDate: %s", checkDate)
  #sys.exit(1)
  try:
  ...
  if len(mailFields['attachmentFile']) != 0:			# check whether any attachment has come with old combo
    ...
    #if row == None:						# i.e. there wasn't any attachment for old combo. Save the new attachment(s)
    if row == ():
  ...
  if len(mailFields['links']) != 0:						# check whether any attachment has come with old combo
    ...
    #if row == None:							# i.e. there wasn't any link for old combo. Save the new link(s)
    if row == ():
